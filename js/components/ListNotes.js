...
const ListNotes = ( { lstElement, type } )=>{
    ...
    createLocalStorage( 'NotesOrder', 'month' )
    createLocalStorage( 'Notes', JSON.stringify( { note : false, trash : false, share : false, offline : [] } ) ) 
    ...
    const contentList =( lst = [] )=>{
        ...
        return  (`
            ...
            ` : `
                ${ lst.map( (data, index) =>{
                    const order = getLocalStorage( 'NotesOrder', 'month' ) 
    ...
    const loadList = async ()=>{ 
        ...
        const Lst = JSON.parse( getLocalStorage( 'Notes', '[{}]' ) ) 
        ...
        if( document.body.contains( lstElement ) ){
            ...
            if( JSON.stringify( lst ) !== JSON.stringify( Lst[ type ] ) ){
                ...
                Lst[ type ] = lst.slice( 0, type === 'offline' ? 50 : 25 ) 
