...
const OpcDateNote =( loadList )=>{
    ...
    const htmlOptionsChangeDateList =()=>{
        const lstOrderNotes = getLocalStorage( 'lstOrderNotes', 'month' )

        return [ 'day', 'month', 'year' ].map( ( data )=>{

            return (`
                <button type="button" class="button__yuHmx after${ data === lstOrderNotes ? ' select ' : ' ' }LFrO7mL2NZVAXtViOa9h" data-date="${ data }" >
                    <span> ${ data } </span>
                    ${ data === lstOrderNotes ? '<i class="fa-solid fa-circle-check"></i>' : '' }
                </button>
            `)
    ...
    element.addEventListener( 'click', ({target})=> {
        ...
        if( target.classList.contains( 'LFrO7mL2NZVAXtViOa9h' ) ) {
            localStorage.setItem( 'lstOrderNotes', target.dataset.date )
            loadList()
            return hideElement()
        } 
